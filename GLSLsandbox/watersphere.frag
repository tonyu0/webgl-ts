...
float map(vec3 p, float itime) {
	...
	float d, h = 0.0;    
	for(int i = 0; i < 8; i++) {        
		d = octave((p.xz+stime)*freq,choppy);
		d += octave((p.xz-stime * 1.3)*freq,choppy);
		d += octave((p.xz+stime * 2.2)*freq,choppy);
		h += d * amp;        
		p.xz *= vec2(1.5,-1.71);
		freq *= 3.5;
		amp *= .15;
...
vec3 trace(vec3 ori, vec3 dir, float itime) {  
	...
	if(hx > 0.0) return vec3(tx);   // <---- bara för demo syfte
...
vec3 get_normal(vec3 p, float eps, float itime) {
    vec3 n;
    n.y = map(p, itime);    
...
vec3 diffuse(vec3 n,vec3 l,float p) {
    return vec3(0.1,0.19,0.22) + pow(dot(n,l) * 0.4 + 0.6,p) * SEA_WATER_COLOR;
}
...
vec3 get_sea_color(vec3 p, vec3 n, vec3 l, vec3 eye, vec3 dist) {  
    ...
    vec3 refracted = diffuse(n,l,80.0);
...
void main( void ) {
	...
	// cam dir for trace
	vec3 ang = vec3(0.0, .5, .0);
	dir = normalize(dir) * from_euler(ang);

	float now = time;
	vec3 p = trace(ori,dir, now);
	vec3 dist = p - ori;
	vec3 n = get_normal(p, dot(dist,dist) * (.2 / resolution.x), now);
