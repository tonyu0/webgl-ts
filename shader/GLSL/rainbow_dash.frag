...
// Rainbow Dash rendered by mapping signs of improper equations of graphic
// primitives, mainly lines, circles, and ellipses. (c) mniip 2014
...
bool ellipse(vec2 p, vec2 a, vec2 r) {
  return (p.x - a.x) * (p.x - a.x) / r.x / r.x +
             (p.y - a.y) * (p.y - a.y) / r.y / r.y >
         1.0;
}
...
bool mane(inout vec3 c, vec2 p) {
  ...
  if (!A && !B && (C || (!D && E) || (!F && G) || (!H || (I && !J)) && !K) ||
    ...
    if ((!M && N) || ((!O && P || !Q && !M) && R) || (!S && !M && !T) ||
        (U && !T && V)) {
      if (circle(p, vec2(491, 84) - off * .5, 764.))
        c = COLOR_MANE1;
      else if (circle(p, vec2(686, 204) - off * .2, 576.))
        c = COLOR_MANE2;
      else
        c = COLOR_MANE3;
    } else
...
bool face(inout vec3 c, vec2 p) {
  ...
  if (!A && B || !C && D) {
    ...
    bool K = ellipse(p * (sin(time / 1.75) / 85. + 1.), vec2(253, 192),
                     vec2(102, 70));
...
#define SQR(q) ((q) * (q))
  bool A = SQR((p.x - 590.0) / 150.0 - (p.y - 378.0) / 900.0) +
               SQR((p.y - 378.0) / 180.0) <
           1.0;
  if (A) {
    bool B = SQR((p.x - 593.0) / 149.0 - (p.y - 382.0) / 900.0) +
                 SQR((p.y - 361.0) / 180.0) <
             1.0;
    if (B) {
      ...
      if (C)
        ...
        if (D && E) {
          bool F =
              ellipse(p - off * 50., vec2(580, 360) + d * 1.3, vec2(71, 126));
          if (F)
            c = mix(COLOR_IRIS1, COLOR_IRIS2,
                    SQR((p.y - 360.0 - d.y) / 252.0 + 0.5));
          else
            c = COLOR_BLACK;
        } else
          c = COLOR_WHITE;
  ...
  bool H = ellipse(vec2(p.x + p.y * 0.3, p.x * -0.3 + p.y), vec2(890, 260),
                   vec2(40, 6));
  bool I = ellipse(vec2(p.x + p.y * 0.5, p.x * -0.5 + p.y), vec2(990, 175),
                   vec2(40, 7));
  if (!G || !H || !I) {
    c = COLOR_BLACK;
    return true;
  ...
  if (!A && !B && !C || !D) {
    ...
    if (!E) {
      ...
      if (F)
        ...
        if (D && E) {
          bool F =
              ellipse(p - off * 50., vec2(290, 360) + d * 1.3, vec2(42, 126));
          if (F)
            c = mix(COLOR_IRIS1, COLOR_IRIS2,
                    SQR((p.y - 360.0 - d.y) / 252.0 + 0.5));
          else
...
float sinslope(float t) { return sin(t) - sin(t - 0.01); }
...
void main() {
  ...
  vec3 c = vec3(1, 1, 1);
  ear(c, p) || mane(c, p) || mane2(c, p) ||
      eye1(c, p, vec2(off * 1.6, off * .5)) ||
      eye2(c, p, vec2(off * .4, off * .5)) || face(c, p) || neck(c, p);
