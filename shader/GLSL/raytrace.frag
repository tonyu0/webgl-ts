...
float hash1() { return fract(sin(seed += 1.0)); }

vec2 hash2() { return fract(sin(vec2(seed, seed += 1.0))); }

vec3 hash3() { return fract(sin(vec3(seed, seed += 1.0, seed += 2.0))); }
...
vec3 nPlane(in vec3 ro, in vec4 obj) { return obj.xyz; }
...
void initLightSphere(float time) {
  lightSphere = vec4(3.0 + 2. * sin(time), 2.8 + 2. * sin(time * 0.9),
                     3.0 + 4. * cos(time * 0.7), .5);
}
...
bool matIsSpecular(const in float mat) { return mat > 4.5; }

bool matIsLight(const in float mat) { return mat < 0.5; }

//-----------------------------------------------------
// brdf
//-----------------------------------------------------

vec3 getBRDFRay(const in vec3 n, const in vec3 rd, const in float m,
                inout bool specularBounce) {
...
vec3 traceEyePath(in vec3 ro, in vec3 rd, const in bool directLightSampling) {
  ...
  for (int j = 0; j < EYEPATHLENGTH; ++j) {
    ...
    if (matIsLight(res.y)) {
      ...
      //   basecol = vec3(0.);	// the light has no diffuse component, therefore
      //   we can return col
      return tcol;
    }
    ...
    if (directLightSampling) {
      ...
      if (!specularBounce && j < EYEPATHLENGTH - 1 &&
          !intersectShadow(ro, nld, length(ld))) {

        float cos_a_max =
            sqrt(1. - clamp(lightSphere.w * lightSphere.w /
                                dot(lightSphere.xyz - ro, lightSphere.xyz - ro),
                            0., 1.));
        float weight = 2. * (1. - cos_a_max);

        tcol +=
            (fcol * LIGHTCOLOR) * (weight * clamp(dot(nld, normal), 0., 1.));
      }
    }
  }
  return tcol;
